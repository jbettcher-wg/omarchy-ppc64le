...
def write_makepkg_conf(path, pkgdest, srcdest, logdest, make_jobs=None):
    ...
    with open(path, "w") as fh:
        fh.write(
            ...
            "# not beside the /etc/makepkg.conf we just sourced. Without these two\n"
            "# lines /etc/makepkg.conf.d/*.conf never applied to a bq build, so\n"
            ...
            "for _d in /etc/makepkg.conf.d/*.conf; do\n"
            "  [ -r \"$_d\" ] && source \"$_d\"\n"
            "done\n"
            "unset _d\n"
            'PKGDEST="%s"\n'
